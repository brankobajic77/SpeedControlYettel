...
print("=========================")
# ==== ADMIN ROUTES (router) – robustni blok za list/stats/export/clear + meta ====
from fastapi import APIRouter
from fastapi.responses import StreamingResponse
import io, csv
from collections import Counter
...
@admin.get("/reports/meta")
def reports_meta():
    p = os.path.abspath(REPORTS_FILE)
    exists = os.path.exists(p)
    size = os.path.getsize(p) if exists else 0
    return {"path": p, "exists": exists, "size": size}

@admin.get("/reports")
def list_reports2(limit: int = 200, since: str | None = None, segment: str | None = None):
    items = _load_reports2()
    if since:
        sdt = _parse_iso2(since)
        if sdt:
...
# Prikači router pod /v1/traffic
app.include_router(admin, prefix="/v1/traffic", tags=["admin"])

# (Re)dodaj whoami sa radnim direktorijumom i punom putanjom do fajla
import socket
@app.get("/whoami")
def whoami():
    import os
    return {
        "file": __file__,
        "cwd": os.getcwd(),
        "reportsFile": os.path.abspath(REPORTS_FILE),
        "time": datetime.utcnow().isoformat() + "Z",
    }

# Ispiši sve rute (vidi u konzoli da su zaista registrovane)
print("=== REGISTERED ROUTES ===")
for r in app.routes:
    try:
        methods = ",".join(sorted(r.methods))
